...
var smoke_program: any;

var smoke_loc_position: any;
var smoke_loc_rotation: any;
var smoke_loc_alpha: any;
var smoke_loc_size: any;
var smoke_loc_resolution: number;
var smoke_loc_fov: any;
var smoke_loc_texture: any;

var particle_array: Smoke_particle[];

var smoke_texture:any;
var vbo: any;
var smoke_attrib_vert: any;
...
class Smoke_particle{
	...
	rotation: number = 0; //randomize rotations so it looks less uniform
	life:number = 1000; //life in seconds
	using: boolean = false;
	rel_pos = new Vector3(); //used for depth sorting, to make transparency work

	being_used():boolean{return this.using}
	reset(pos:Vector3){
		let rand_offset = .5;
	...
	get_rotation():number{return this.rotation;}

	get_alpha_and_size():number[]{
...
function init_smoke_shader(gl: any){
...
function update_smoke(dt:number){
	var p = 0
...
function draw_smoke(gl:any, dt:number, fov:number, resolution:number[], camera_position:Vector3, camera_orientation:Quaternion, ship:Ship){
	...
	if (ship.alive()){
		...
		if (Math.random() < dt * 1 * vel){ //eg 300 per second if 300m/s
			...
			if (Math.random() < .5) {side = new Vector3(-3,0,5);} //left
			else {side = new Vector3(3,0,5);} //right
	...
	particle_array.sort(function(a:Smoke_particle,b:Smoke_particle):number{return +(a.rel_pos.get_z() > b.rel_pos.get_z())*2-1})
