...
var model_program: any;
var model_attrib_position: any;
var model_attrib_texCoord: any;
var model_attrib_normal: any;
var palette_texture: any;
var model_loc_modelOrientation: number;
var model_loc_modelPosition: number;
var model_loc_orientation: number;
var model_loc_position: number;
var model_loc_emission_scalar: number;
var model_loc_resolution: number;
var model_loc_texture: number;
var model_loc_fov: number;

var obstacle_models: Model[];
var obstacle_positions:[number, Vector3, Quaternion][]; //array of model_id, posx, posy, posz
...
class Model{
	model_data: Float32Array = new Float32Array([]);
	vbo: number = 0;

	has_grid = false;
	collision_grid:string = "";
	dimensions:Vector3 = new Vector3();
	max_range = 0;

	constructor(gl:any, address:string, y_offset:number, scale:number, collision_address:string = "", dimensions:Vector3 = new Vector3()){
		...
		let texture_coords : [number, number][] = [];
	
		let obj_data: string = loadFile(address);
	
		let lines: string[] = obj_data.split('\n');
		for (let l = 0; l < lines.length; l++){
			let words: string[] = lines[l].split(' ');
			if (words.length > 0){
				if (words[0] == 'vn'){ normals.push(         [parseFloat(words[1]), parseFloat(words[2]), parseFloat(words[3]) ]); } //z components negative
				if (words[0] == 'vt'){ texture_coords.push(  [parseFloat(words[1]), parseFloat(words[2])                       ]); }
				if (words[0] == 'v' ){ vertices.push(        [parseFloat(words[1])*scale, (parseFloat(words[2])+y_offset)*scale, parseFloat(words[3])*scale ]); }
			}
		}
		
		let model_data: number[] = [];
		...
		for (let l = 0; l < lines.length; l++){
			...
			if (words.length > 0){
				if (words[0] == 'f'){
					for (let v = 1; v < 4; v++){
						let vertex_data: string[] = words[v].split('/');
		...
		if (collision_address != ""){
			...
			if (this.collision_grid == "empty?"){alert('cant find: '+collision_address);}
		}
	}
	...
	check_point_inside(relative_point:Vector3):boolean{
...
function init_model_shader(gl: any){
...
function check_model_collision(dt:number, ship:Ship) {
...
function check_model_bullet_collision(dt:number, bullets:Bullet[]) {
...
function draw_models(gl: any, fov:number, resolution: number[], camera_position: Vector3, camera_orienation:Quaternion, ship1:Ship, ship2:Ship){
